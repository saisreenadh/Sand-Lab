...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    False
    >>> is_move_ok(grid, 0, 0, -1, -2)
    False
    >>> is_move_ok(grid, 0 , 0, -2, -1)
    False
    ...
    if grid.get(x_to, y_to) != None:
        return False
    if x_to == x_from - 1 or x_to == x_from + 1:
        if y_to == y_from + 1:
...
def do_gravity(grid, x, y):
    ...
    if grid.get(x, y) != 's':
        return grid
    elif is_move_ok(grid, x, y, x, y + 1):
...
def do_whole_grid(grid, brownian):
    ...
    (tests and code TBD)
    >>> grid = Grid.build([[None, None, None], ['s', 's', 's'], [None, None, None]])
    >>> do_whole_grid(grid, 0)
    [[None, None, None], [None, None, None], ['s', 's', 's']]
